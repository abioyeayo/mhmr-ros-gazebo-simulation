...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, args, device, action=None, action_masks=None):
        if args.distinct_actor_critic:
            logits = self.actor(x)
        else:
        ...
        if args.action_masks:
            ...
            probs = CategoricalMasked(logits=logits, masks=action_masks, device=device)
        if action is None:
            action = probs.sample()
        return (
            action,
            probs.log_prob(action),
            probs.entropy(device),
